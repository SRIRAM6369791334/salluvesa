...
@section('content')
  ...
  </section> <section class="premium-contact-section">
            ...
            <div class="row cs_gap_y_30">
              ...
              <div class="col-lg-4 col-md-6">
                <div class="cs_shop-card" style="border: 1px solid #E5E5E5; padding: 15px; border-radius: 12px; transition: transform 0.3s ease;">
                  ...
                  <style>
                    .design-preview-box:hover .design-overlay { opacity: 1 !important; }
                    .design-preview-box:hover { transform: translateY(-5px); }
                  </style>
  ...
  <style>
    ...
    .premium-hero-section{min-height:400px;display:flex;align-items:center;background:linear-gradient(135deg,#1C30A3 0%,#2541C8 50%,#3B5FE0 100%);position:relative;padding:120px 0 180px}
    .hero-particles{position:absolute;top:0;left:0;width:100%;height:100%;overflow:hidden;z-index:1}
    .hero-gradient-overlay{position:absolute;top:0;left:0;width:100%;height:100%;background:radial-gradient(circle at 20% 50%,rgba(102,126,234,.3) 0%,transparent 50%),radial-gradient(circle at 80% 80%,rgba(240,147,251,.3) 0%,transparent 50%);z-index:1}
    .hero-content{position:relative;z-index:2}
    .hero-badge{display:inline-flex;align-items:center;gap:8px;background:rgba(255,255,255,.2);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,.3);padding:10px 24px;border-radius:50px;color:white;font-size:14px;font-weight:500;margin-bottom:30px}
    .premium-hero-title{font-size:56px;font-weight:900;font-family:'Merriweather',serif;color:white;margin:0 0 20px 0;line-height:1.2}
    .hero-subtitle{font-size:18px;color:rgba(255,255,255,.9);margin:0;max-width:600px;margin-left:auto;margin-right:auto}
    ...
    .premium-contact-section{position:relative;overflow:hidden}
    .animated-gradient-bg{position:absolute;top:0;left:0;width:100%;height:100%;background:linear-gradient(135deg,#f8f9ff 0%,#e8ecff 25%,#f0f4ff 50%,#e8f0ff 75%,#f8faff 100%);background-size:400% 400%;animation:gradientShift 15s ease infinite;z-index:0}
    .premium-contact-section > *:not(.animated-gradient-bg){position:relative;z-index:1}
    ...
    .account-menu-item{display:flex;align-items:center;gap:12px;padding:14px 18px;color:#5E5E5E;text-decoration:none;border-radius:10px;transition:all 0.3s ease;font-weight:500;margin-bottom:4px}
    .account-menu-item i{width:20px;text-align:center;font-size:16px;transition:all 0.3s ease}
    .account-menu-item:hover{background:linear-gradient(135deg,rgba(28,48,163,0.08) 0%,rgba(37,65,200,0.08) 100%);color:#1C30A3;transform:translateX(5px)}
    .account-menu-item:hover i{color:#1C30A3}
    .account-menu-item.active{background:linear-gradient(135deg,#1C30A3 0%,#2541C8 100%);color:#fff;box-shadow:0 4px 15px rgba(28,48,163,0.3)}
    .account-menu-item.active i{color:#fff}
    .account-menu-item.logout-link:hover{background:linear-gradient(135deg,rgba(220,53,69,0.1) 0%,rgba(220,53,69,0.15) 100%);color:#dc3545}
    .account-menu-item.logout-link:hover i{color:#dc3545}
            ...
            $.ajax({
                ...
                success: function(res) {
                    if(res.success) {
                        ...
                        hideLoader();
                        Swal.fire("{{ gt('Errro') }}", res.message, 'error');
                        btn.prop('disabled', false).text(originalText);
                    }
                },
                error: function(xhr) {
